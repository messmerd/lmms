...
#define READ_DWORD( x ) \
		...
		auto data = std::vector<SampleFrame>{};
		data.reserve(frames);
		for (f_cnt_t frame = 0; frame < frames; ++frame)
		{
			data.emplace_back(wave_samples[frame]);
		}

		auto psample = std::make_shared<Sample>(std::make_shared<SampleBuffer>(std::move(data), sample_rate));
