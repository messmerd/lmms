		...
		if (track->type() == Track::InstrumentTrack)
		{
					...
					node = node.nextSibling())
			{
				...
				else if (node.nodeName() == "pattern") //TODO: rename to "midiClip"
				{
					...
					midiClip.write(node, base_pitch, base_volume, base_time);
				}
	...
	for (Track* track : tracksBB)
	{
				...
				node = node.nextSibling())
		{
			...
			else if (node.nodeName() == "pattern") //TODO: rename to "midiClip"
			{
				...
				midiClip.write(node, base_pitch, base_volume, 0);
				...
				for (pair<int, int> &p : plist)
				{
					...
					{
						writeBBClip(midiClip, bbClip, len, st.back().first, pos, st.back().second);
						pos = st.back().second;
						st.pop_back();
					}
					if (not st.empty() and st.back().second <= p.second)
					{
						writeBBClip(midiClip, bbClip, len, st.back().first, pos, p.first);
						pos = p.first;
						while (!st.empty() && st.back().second <= p.second)
						{
							st.pop_back();
						}
					}
...
void MidiExport::writeBBClip(Clip& src, Clip& dst,
