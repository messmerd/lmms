...
void MidiFile::Section::writeBytes(vector<uint8_t> bytes,
		vector<uint8_t> *v)
{
...
void MidiFile::Track::addName(const string &name, uint32_t time)
{
	Event event;
...
void MidiFile::Track::writeEventsToBuffer()
{
	...
	vector<Event> eventsSorted = m_events;
...
void MidiFile::Track::writeSingleEventToBuffer(Event &event)
{
	...
	case MidiFile::Event::TRACK_NAME:
	{
		...
		vector<uint8_t> bytes(event.m_trackName.begin(),
